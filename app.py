...
def generate_portfolio_pdf(portfolio, holdings, history_data=None, alerts=None,
    ...
    """Generate a branded Alpha Report PDF for a portfolio."""
    ...
    cell_style = ParagraphStyle("CellWrap", parent=styles["Normal"], fontSize=8,
                                 textColor=dark_text, leading=10)
    green_cell = ParagraphStyle("CellGreen", parent=cell_style, textColor=green_c)
    red_cell = ParagraphStyle("CellRed", parent=cell_style, textColor=red_c)
    ...
    cover_title = ParagraphStyle("CoverTitle", parent=title_style, fontSize=36, alignment=1)
    cover_sub = ParagraphStyle("CoverSub", parent=body_style, fontSize=16, alignment=1,
                                textColor=colors.HexColor("#6B7280"))
    cover_name = ParagraphStyle("CoverName", parent=body_style, fontSize=14, alignment=1)
    cover_user = ParagraphStyle("CoverUser", parent=body_style, fontSize=11, alignment=1,
                                 textColor=colors.HexColor("#6B7280"))
    cover_date = ParagraphStyle("CoverDate", parent=small_style, fontSize=10, alignment=1)
 
    story.append(Paragraph("KORDENT", cover_title))
    story.append(Spacer(1, 4*mm))
    story.append(HRFlowable(width="30%", thickness=2, color=brand_blue,
                            spaceAfter=8, spaceBefore=4, hAlign="CENTER"))
    story.append(Paragraph("Alpha Report", cover_sub))
    story.append(Spacer(1, 20*mm))
    story.append(Paragraph(portfolio.get("name", "Portfolio"), cover_name))
    if user_name:
        story.append(Paragraph(user_name, cover_user))
    story.append(Spacer(1, 6*mm))
    story.append(Paragraph(today_str, cover_date))
    ...
    summary_rows = [
        ["Current Value", f"Rs. {current_val:,.0f}"],
        ["Total Invested", f"Rs. {total_invested:,.0f}"],
        ["P&L", f"Rs. {pnl:+,.0f} ({return_pct:+.1f}%)"],
    ]
    if port_xirr is not None:
        summary_rows.append(["XIRR", f"{port_xirr:+.1f}%"])
    if nifty_xirr is not None:
        summary_rows.append(["Nifty XIRR", f"{nifty_xirr:+.1f}%"])
    if alpha_xirr is not None:
        summary_rows.append(["Alpha vs Nifty", f"{alpha_xirr:+.1f}%"])
    summary_rows.extend([
        ["Monthly SIP", f"Rs. {sip:,.0f}"],
        ["Investor Type", portfolio.get("investor_type", "\u2014")],
        ["Time Horizon", portfolio.get("time_horizon", "\u2014")],
        ["Review", f"Every {portfolio.get('review_freq', 90)} days"],
        ["Next Review", str(portfolio.get("next_review_date", "\u2014"))],
    ])
    ...
    if goal_data and goal_data.get("status"):
        ...
        if _gs["status"] == "on_track":
            summary_rows.append(["Goal", f"On track \u2014 Rs. {_target:,.0f} target"])
        elif _gs["status"] == "behind":
            summary_rows.append(["Goal", f"Behind by Rs. {abs(_gs['gap']):,.0f}"])
        elif _gs["status"] == "ahead":
            summary_rows.append(["Goal", f"Ahead by Rs. {abs(_gs['gap']):,.0f}"])
        elif _gs["status"] == "achieved":
            summary_rows.append(["Goal", f"Achieved! Target was Rs. {_target:,.0f}"])
    ...
    # ══════════════════════════════════════
    # HOLDINGS TABLE (enhanced: CMP + P&L)
    # ══════════════════════════════════════
    if holdings:
        story.append(Paragraph("Holdings", heading_style))
 
        header = ["Stock", "Shares", "Entry", "CMP", "Invested", "Value", "P&amp;L", "Alloc %"]
        rows = [header]
        ...
        for h in holdings:
            ...
            pnl_para = Paragraph(pnl_text, green_cell if h_pnl >= 0 else red_cell)
        ...
        rows.append(["Total", "", "", "", f"Rs. {_tot_inv:,.0f}", f"Rs. {_tot_val:,.0f}", _tot_pnl_text, ""])
 
        col_widths = [36*mm, 12*mm, 18*mm, 18*mm, 20*mm, 20*mm, 28*mm, 14*mm]
    ...
    # ══════════════════════════════════════
    if chart_buf:
        story.append(Paragraph("Portfolio Growth", heading_style))
        chart_buf.seek(0)
        story.append(RLImage(chart_buf, width=160*mm, height=70*mm))
        story.append(Spacer(1, 4*mm))
    ...
    # ══════════════════════════════════════
    if goal_chart_buf:
        story.append(Paragraph("Goal Projection", heading_style))
        goal_chart_buf.seek(0)
        story.append(RLImage(goal_chart_buf, width=160*mm, height=60*mm))
        story.append(Spacer(1, 4*mm))
    ...
    if holdings:
        ...
        if total_val_sec > 0:
            ...
            if sector_weights:
                story.append(Paragraph("Sector Exposure", heading_style))
                sec_header = ["Sector", "Weight", "1M Momentum"]
                sec_rows = [sec_header]
                for sec, val in sorted(sector_weights.items(), key=lambda x: -x[1]):
                    weight_pct = val / total_val_sec * 100
                    momentum = sector_data.get(sec) if sector_data else None
                    if momentum is not None:
                        mom_str = f"{momentum:+.1f}%"
                    else:
                        mom_str = "\u2014"
                    sec_rows.append([sec, f"{weight_pct:.1f}%", mom_str])
 
                sec_table = Table(sec_rows, colWidths=[60*mm, 30*mm, 30*mm])
                # Color momentum cells
                sec_style_cmds = [
                    ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
                    ("FONTSIZE", (0, 0), (-1, -1), 9),
                    ("BACKGROUND", (0, 0), (-1, 0), brand_blue),
                ...
                # Color individual momentum cells green/red
                if sector_data:
                    for row_idx, (sec, _) in enumerate(sorted(sector_weights.items(), key=lambda x: -x[1])):
                        m = sector_data.get(sec)
                        if m is not None:
                            c = green_c if m >= 0 else red_c
                            sec_style_cmds.append(("TEXTCOLOR", (2, row_idx + 1), (2, row_idx + 1), c))
 
                sec_table.setStyle(TableStyle(sec_style_cmds))
    ...
    if score_data and holdings:
        ...
        for h in holdings:
            ...
            sc = sd.get("score", "\u2014")
            _g = "Pass" if sd.get("graham_pass") else "Fail"
            _gb = "Pass" if sd.get("greenblatt_pass") else "Fail"
            _d = "Pass" if sd.get("dorsey_pass") else "Fail"
            _t = "Pass" if sd.get("trajectory_pass") else "Fail"
            _q = "Pass" if sd.get("quality_pass") else "Fail"
            sc_rows.append([
                Paragraph(h.get("name", ticker), cell_style),
                str(sc), _g, _gb, _d, _t, _q
            ])
 
        sc_table = Table(sc_rows, colWidths=[40*mm, 14*mm, 20*mm, 22*mm, 16*mm, 22*mm, 16*mm])
        sc_style_cmds = [
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
            ("FONTSIZE", (0, 0), (-1, -1), 8),
            ("BACKGROUND", (0, 0), (-1, 0), brand_blue),
        ...
        for row_i, h in enumerate(holdings):
            ...
            for col_i, key in enumerate(["graham_pass", "greenblatt_pass", "dorsey_pass", "trajectory_pass", "quality_pass"]):
                c = green_c if sd.get(key) else red_c
                sc_style_cmds.append(("TEXTCOLOR", (col_i + 2, row_i + 1), (col_i + 2, row_i + 1), c))
 
        sc_table.setStyle(TableStyle(sc_style_cmds))
    ...
    if alerts:
        ...
        for a in alerts:
            icon = {"danger": "DANGER", "opportunity": "OPPORTUNITY",
                    "review_due": "REVIEW DUE"}.get(a.get("alert_type", ""), "ALERT")
            safe_hl = str(a.get("headline", "")).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            story.append(Paragraph(f"[{icon}] {safe_hl}", body_style))
        story.append(Spacer(1, 4*mm))
    ...
    # ── Build with branded footer on every page ──
    doc.build(story, onFirstPage=_footer, onLaterPages=_footer)
    buffer.seek(0)
    return buffer.getvalue()

def _plotly_to_png(fig, width=800, height=350):
    """Render a Plotly figure to PNG BytesIO via kaleido. Returns None on failure."""
    try:
        import io as _io
        buf = _io.BytesIO()
